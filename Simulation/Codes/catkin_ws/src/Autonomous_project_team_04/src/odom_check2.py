...
####################################################################################################################################################################
## ROS Publisher Code
pub1 = rospy.Publisher('/steer_bot/Odom_Values', ModelStates, queue_size=10) #Identify the publisher "pub1" to publish on topic "/Odom_Values" to send message of type "Odometry"
Odom_Noise = Float64() #Identify msg variable of data type Odometry
...
def noisy_add(position, velocity):
	...
	thetanoise = position[5] + random.gauss(0,0.01)		##Noisy data calculated at z

	position_noisy = [xnoise, ynoise, position[2], position[3], position[4], thetanoise]##Store the noisy position in array))
	count = count+1
...
def callback(data):
	...
	if data.name.__contains__("steer_bot"):
		...
		Odom_Noise.append(round(position_noisy[0],4))
		Odom_Noise.append(round(position_noisy[1],4)) 
		Odom_Noise.append(round(position_noisy[2],4))
		(x, y, z, w) = euler_to_quaternion(position_noisy[3], position_noisy[4], position_noisy[5]) #Transform Euler to Quaternian angles
		Odom_Noise.append(round(x,4))  
		Odom_Noise.append(round(y,4)) 
		Odom_Noise.append(round(z,4))
		Odom_Noise.append(round(w,4))
		Odom_Noise.append(round(velocity_noisy[0],4)) 
		Odom_Noise.append(round(velocity_noisy[1],4)) 
		Odom_Noise.append(round(velocity_noisy[2],4)) 
		Odom_Noise.append(round(velocity_noisy[3],4)) 
		Odom_Noise.append(round(velocity_noisy[4],4))
		Odom_Noise.append(round(velocity_noisy[5],4))
...
def callback_Init(data):
	...
	if data.name.__contains__("steer_bot"):
		...
		[position_noisy, velocity_noisy] = noisy_add(position_0, velocity_0)
		Odom_Noise.append(round(position_noisy[0],4))
		Odom_Noise.append(round(position_noisy[1],4)) 
		Odom_Noise.append(round(position_noisy[2],4))
		(x, y, z, w) = euler_to_quaternion(position_noisy[3], position_noisy[4], position_noisy[5]) #Transform Euler to Quaternian angles
...
while 1 and not rospy.is_shutdown():
	#print("posex = " + str(Odom_Noise.pose[0]))
	#print("posey = " + str(Odom_Noise.pose[1]))
	pass
###################################################################################################################################################################
